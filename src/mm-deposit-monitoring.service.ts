...
@Injectable()
export class MMDepositMonitoringService implements OnModuleInit {

  private readonly logger = new Logger(MMDepositMonitoringService.name);
  ...
  onModuleInit() {
    const monitor = Monitors.builder({
      ...
      .poll(
        async (subscribers) => this.investinService.getMMPendingInvestments(subscribers),
      ...
      .dialectSdk(
        ({ value, context }) => {
          const message: string = `You have recieved a deposit of $${value.map(f => (Number(f.amount) / 10 ** 6).toFixed(2)).join(',')}!`;

          this.logger.log(
            `${context.origin.subscriber} has recieved a deposit of $${value.map(f => (Number(f.amount) / 10 ** 6).toFixed(2)).join(',')}!`,
          );
          return {
